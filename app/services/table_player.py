...
async def add_player_at_table(session, table_id, player_id):
    ...
    if not any(
        gp.player_id == player_id and gp.status == Status.JOINED
        for gp in table.game.players
    ):
    ...
    if existing:
            raise ApplicationException(f"Please leave table #{existing.table.number} before joining a new one", 400)
